...
import com.peknight.demo.frontend.vue.introduction.IntroductionPage
  ...
  private[this] val htmlRoutes: HttpRoutes[IO] = HttpRoutes.of[IO] {
    ...
    case req @ GET -> Root / path if Set(".js", ".map").exists(path.endsWith) =>
      StaticFile.fromPath(file.Path(s"./demo-front-end/js/target/scala-3.2.1/demo-front-end-opt/$path"), Some(req))
        .orElse(StaticFile.fromPath(file.Path(s"./demo-front-end/module/demo-vue/target/scala-3.2.1/demo-front-end-module-demo-vue-opt/$path"), Some(req)))
        .getOrElseF(NotFound())
  }
