...
object DequeueApp extends IOApp.Simple:
  ...
  def covariant(list: List[Int]): IO[List[Long]] =
    for
      ...
      // kind projector DequeueSource[IO, *]
      qOfLongs: DequeueSource[IO, Long] = Functor[[A] =>> DequeueSource[IO, A]].map(q)(_.toLong)
  ...
  def contravariant(list: List[Boolean]): IO[List[Int]] =
    for
      q <- Dequeue.bounded[IO, Int](10)
      qOfBools: DequeueSink[IO, Boolean] = Contravariant[[A] =>> DequeueSink[IO, A]].contramap[Int, Boolean](q)(b => if b then 1 else 0)
