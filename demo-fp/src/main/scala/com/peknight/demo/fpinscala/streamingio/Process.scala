...
import com.peknight.demo.fpinscala.streamingio.Process.*
...
trait Process[F[_], O]:
  ...
  // Exercise 15.10
  def runLog(using F: MonadCatch[F]): F[IndexedSeq[O]] =
  ...
  def |>[O2](p2: Process1[O, O2]): Process[F, O2] = p2 match
    ...
    case Await(req, recv) => this match
      ...
      case Emit(h, t) => t |> Try(recv.asInstanceOf[Either[Throwable, O] => Process1[O, O2]](Right(h)))
      case Await(req0, recv0) => await(req0)(recv0.andThen(_ |> p2))


  // 给最外层的Await传递Kill异常，但忽略余下的输出
  @tailrec final def kill[O2]: Process[F, O2] = this match
    case Await(_, recv) =>
...
object Process:
  ...
  def runLog[O](src: Process[IO, O]): IO[IndexedSeq[O]] = Suspend(Par.lazyUnit {
    given E: ExecutorService = Executors.newFixedThreadPool(4)
    def go(cur: Process[IO, O], acc: IndexedSeq[O]): IndexedSeq[O] = cur match
      case Emit(h, t) => go(t, acc :+ h)
      case Halt(End) => acc
      case Halt(err) => throw err
      case Await(req, recv) =>
        val next = try recv(Right(unsafePerformIO(req))) catch case err: Throwable => recv(Left(err))
        go(next, acc)
  ...
  def filter[I](f: I => Boolean): Process1[I, I] = await1[I, I](i => if f(i) then emit(i) else halt1).repeat

  def take[I](n: Int): Process1[I, I] = if n <= 0 then halt1 else await1[I, I](i => emit(i, take(n - 1)))

  def takeWhile[I](f: I => Boolean): Process1[I, I] = await1(i => if f(i) then emit(i, takeWhile(f)) else halt1)

  def dropWhile[I](f: I => Boolean): Process1[I, I] = await1(i => if f(i) then dropWhile(f) else emit(i, id))
