...
object Sliceable extends Parsers[Parser]:
  ...
  override def run[A](p: Parser[A])(input: String): Either[ParseError, A] =
    p(ParseState(Location(input), false)).extract(input)

  override given string: Conversion[String, Parser[String]] = (s: String) => state =>
    val i = firstNonmatchingIndex(state.loc.input, s, state.loc.offset)
    if i == -1 then
      if state.isSliced then Slice(s.length) else Success(s, s.length)
    else Failure(state.loc.advanceBy(i).toError(s"'$s'"), i != 0)

  override given regex: Conversion[Regex, Parser[String]] = (r: Regex) => state =>
    r.findPrefixOf(state.input) match
      case Some(m) => if state.isSliced then Slice(m.length) else Success(m, m.length)
  ...
  override def map2[A, B, C](p: Parser[A], p2: => Parser[B])(f: (A, B) => C): Parser[C] = state => p(state) match
    ...
    case Slice(n) =>
      ...
      p2(state2) match
        ...
        case Slice(m) =>
          if state.isSliced then Slice(n + m).asInstanceOf[Result[C]]
          else Success(f(state.slice(n).asInstanceOf[A], state2.slice(m).asInstanceOf[B]), n + m)
        // 答案代码没addCommit会导致报错位置不符合预期
        case f @ Failure(_, _) => f.addCommit(n != 0)
    case f @ Failure(_, _) => f
  ...
  override def many[A](p: Parser[A]): Parser[List[A]] = state =>
    if state.isSliced then
      ...
      @tailrec def go(p: Parser[A], offset: Int): Result[List[A]] = p(state.advanceBy(offset)) match
        case Success(a, n) =>
          buf += a
          go(p, offset + n)
        case f @ Failure(_, true) => f
