...
object List:
  ...
  def removeAllOddNumber(as: List[Int]): List[Int] = filter(as)(_ % 2 == 0)
  ...
  // Exercise 3.24
  @tailrec
  def startsWith[A](l: List[A], prefix: List[A])(using CanEqual[A, A]): Boolean = (l, prefix) match
    case (_, Nil) => true
    case (Cons(hp, tp), Cons(hb, tb)) if hp == hb => startsWith(tp, tb)
    case _ => false

  @tailrec
  def hasSubsequence[A](sup: List[A], sub: List[A])(using CanEqual[A, A]): Boolean = sup match
    case Nil => sub == Nil
