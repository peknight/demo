...
object Par:
  ...
  def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] =
    val pars: List[Par[List[A]]] = as.map(asyncF((a: A) => if f(a) then List(a) else List()))
  ...
  def choice[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] = es => if run(es)(cond).get then t(es) else f(es)
  ...
  def choiceMap[K, V](key: Par[K])(choice: Map[K, Par[V]]): Par[V] = es => run(es)(choice(run(es)(key).get))
  ...
  def choiceViaFlatMap[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] = flatMap(cond)(b => if b then t else f)
