...
object MonoidsApp extends App:
  ...
  val intAddition: Monoid[Int] = new Monoid:
    def op(a1: Int, a2: Int): Int = a1 + a2
    def zero: Int = 0

  val intMultiplication: Monoid[Int] = new Monoid:
    def op(a1: Int, a2: Int): Int = a1 * a2
    def zero: Int = 1

  val booleanOr: Monoid[Boolean] = new Monoid:
    def op(a1: Boolean, a2: Boolean): Boolean = a1 || a2
    def zero: Boolean = false

  val booleanAnd: Monoid[Boolean] = new Monoid:
    def op(a1: Boolean, a2: Boolean): Boolean = a1 && a2
  ...
  def dual[A](m: Monoid[A]): Monoid[A] = new Monoid:
    // 反向计算
  ...
  def foldMapV[A, B](as: IndexedSeq[A], m: Monoid[B])(f: A => B): B =
    ...
    if length == 0 then m.zero
    else if length == 1 then f(as(0))
    else
  ...
  def par[A](m: Monoid[A]): Monoid[Par[A]] = new Monoid:
  ...
  val wcMonoid: Monoid[WC] = new Monoid:
  ...
  def count(s: String): Int =
    def wc(c: Char): WC = if c.isWhitespace then Part("", 0, "") else Stub(c.toString)
  ...
  def productMonoid[A, B](ma: Monoid[A], mb: Monoid[B]): Monoid[(A, B)] = new Monoid:
    def op(a1: (A, B), a2: (A, B)): (A, B) = (ma.op(a1._1, a2._1), mb.op(a1._2, a2._2))
    def zero: (A, B) = (ma.zero, mb.zero)

  def mapMergeMonoid[K, V](vm: Monoid[V]): Monoid[Map[K, V]] = new Monoid:
  ...
  def functionMonoid[A, B](mb: Monoid[B]): Monoid[A => B] = new Monoid:
