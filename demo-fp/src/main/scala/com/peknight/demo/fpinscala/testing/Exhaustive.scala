...
import com.peknight.demo.fpinscala.laziness.Stream.Cons
...
object Exhaustive:
  ...
  object Prop:
    ...
    def forAll[A](g: Int => Gen[A])(f: A => Boolean): Prop = Prop {
      (max, n, rng) =>
        ...
        val p: Prop = props.map(p => Prop((max, n, rng) => p.run(max, casesPerSize, rng))).reduceLeft(_ && _)
        p.run(max, n, rng).map {
  ...
  object Gen:
    ...
    /*
     * This is a function we've implemented before. Unfortunately, it does not exist in the standard library.
     * This implementation is uses a foldLeft, followed by a reverse, which is equivalent to a foldRight,
     * but does not use any stack space.
     */
    ...
    def choose(i: Double, j: Double): Gen[Double] = Gen(State(RNG.double).map(d => i + d * (j - i)), unbounded)

    /*
     * Basic idea is add 1 to the result of `choose` if it is of the wrong parity,
     * but we require some special handling to deal with the maximum integer in the range.
     */
     ...
     */
    def union_1[A](g1: Gen[A], g2: Gen[A]): Gen[A] = boolean.flatMap(b => if b then g1 else g2)
    ...
    def weighted[A](g1: (Gen[A], Double), g2: (Gen[A], Double)): Gen[A] =
       ...
       */
      def bools: Stream[Boolean] = randomStream(uniform.map(_ < g1Threshold))(SimpleRNG(302837L))
     ...
     * A value of `true` attempts to pull from `s1`; `false` attempts to pull form `s2`.
