...
object SimpleStreamTransducers {
  ...
  sealed trait Process[I, O] {
    ...
    def feed(in: Seq[I]): Process[I, O] = {
      def go(in: Seq[I], cur: Process[I, O]): Process[I, O] = cur match {
        case Halt() => Halt()
        case Await(recv) => if (in.nonEmpty) go(in.tail, recv(Some(in.head))) else cur
  ...
  object Process {
    ...
    def dropWhile[I](f: I => Boolean): Process[I, I] = await { i => if (f(i)) dropWhile(f) else id }
    ...
    def existsResult[I](f: I => Boolean): Process[I, Boolean] = exists(f) |> takeThrough(!_) |> dropWhile(!_) |> echo.orElse(emit(false))
